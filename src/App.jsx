...
function App() {
  ...
  const [assignedOfficer, setAssignedOfficer] = useState("Not assigned");
  ...
  const deterioration = deteriorationStages[deteriorationIndex];

  const authorityStatus = workflowSteps[workflowIndex];
  ...
  const verifiedCount = potholeStatus === "Verified" ? 4 : 3;
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          <span>BEL Urban Intelligence</span>
        </div>
      </aside>
      ...
      <main className="main-content">
        ...
        {activePage === "Dashboard" && (
          <DashboardPage
            ...
            potholeConfidence={potholeConfidence}
            potholeBuses={potholeBuses}
            potholeStatus={potholeStatus}
            activityLog={activityLog}
            deterioration={deterioration}
            deteriorationIndex={deteriorationIndex}
            advanceDeterioration={advanceDeterioration}
            resetDeterioration={resetDeterioration}
...
function Header({ activePage }) {
  return (
    <header className="topbar">
      <div>
        <p className="eyebrow">CITY COMMAND CENTRE</p>
    ...
    <>
      ...
      <section className="dashboard-grid">
        <div className="panel map-panel">
          <div className="panel-heading">
            <div>
              ...
              <h2>City Observation Map</h2>

              <p
                className={`simulation-status ${
                  simulationRunning ? "running" : "offline"
                }`}
              >
                {simulationRunning
                  ? "Simulation Running"
        ...
        <div className="panel activity-panel">
          <p className="eyebrow">FLEET CONSENSUS</p>
          <h2>Live Activity</h2>
          ...
          <div className="consensus-card">
            ...
            <h3>{potholeConfidence}%</h3>

            <div className="confidence-bar">
              <div
                className="confidence-fill"
                style={{ width: `${potholeConfidence}%` }}
              />
            </div>
      ...
      <section className="memory-grid">
        <div className="panel memory-panel">
          <p className="eyebrow">URBAN DIGITAL MEMORY</p>
          <h2>POT-102 Deterioration Tracking</h2>

          <div className="memory-summary">
            <MemoryItem label="Current Stage" value={deterioration.day} />
            <MemoryItem label="Estimated Size" value={deterioration.size} />
            <MemoryItem label="Growth" value={deterioration.growth} />
            <MemoryItem label="Severity" value={deterioration.severity} />
            <MemoryItem
              label="Priority"
          ...
          <div className="memory-actions">
            <button
              className="primary-button"
              onClick={advanceDeterioration}
              disabled={deteriorationIndex === 2}
            >
        ...
        <div className="panel priority-engine">
          ...
          <div className="priority-factors">
            <Factor label="Road Damage" value={deterioration.severity} />
            <Factor label="Traffic Volume" value="High" />
            <Factor label="Repeat Sightings" value={`${potholeBuses} buses`} />
            <Factor label="Nearby School" value="180 m" />
            <Factor label="Growth" value={deterioration.growth} />
          </div>
        </div>
      </section>
      ...
      <section className="panel authority-panel">
        <div className="panel-heading">
          <div>
            <p className="eyebrow">AUTHORITY WORKFLOW</p>
            <h2>POT-102 Maintenance Action</h2>
          </div>
        ...
        <div className="authority-grid">
          <div className="authority-details">
            <AuthorityRow label="Issue" value="Large Pothole" />
            <AuthorityRow label="Location" value="Connaught Place" />
            <AuthorityRow
              label="Department"
...
function LiveMapPage({
  ...
  return (
    <section className="panel">
      <div className="panel-heading">
        <div>
          <p className="eyebrow">CITY-WIDE OBSERVATION</p>
          <h2>Live GIS Map</h2>
        </div>
...
function UrbanIssuesPage({
  ...
  return (
    <section className="panel">
      <p className="eyebrow">URBAN ISSUE REGISTER</p>
      <h2>Detected City Issues</h2>
...
function FleetPage({ simulationRunning, busPositions }) {
  const buses = Object.keys(busRoutes);

  return (
    <section className="panel">
      <p className="eyebrow">PUBLIC TRANSPORT SENSOR FLEET</p>
      <h2>Fleet Monitoring</h2>

      <div className="fleet-grid">
        {buses.map((busId) => {
          const position =
            busRoutes[busId][busPositions[busId]];

          return (
            <div className="fleet-card" key={busId}>
              <div className="fleet-card-top">
                ...
                >
                  {simulationRunning ? "Active" : "Stopped"}
                </span>
              </div>

              <p>Central Delhi Route</p>

              <div className="fleet-data">
                <span>Latitude</span>
                <strong>{position[0].toFixed(4)}</strong>
              </div>

              <div className="fleet-data">
                <span>Longitude</span>
                <strong>{position[1].toFixed(4)}</strong>
              </div>
          ...
          <tbody>
            <tr>
              <td>
                <PriorityBadge
                  type={
                    deterioration.severity === "Critical"
                      ? "critical"
              ...
              <td>{potholeConfidence}%</td>
              <td>{potholeBuses} buses</td>
            ...
            <tr>
              <td>
                <PriorityBadge type="high">High</PriorityBadge>
              </td>
              <td>Broken Divider</td>
              <td>India Gate</td>
              <td>94%</td>
              <td>3 buses</td>
              <td>Verified</td>
...
function UrbanMap({
  ...
  return (
    <div className="map-wrapper">
      <MapContainer
        center={[28.6265, 77.216]}
        zoom={13}
        scrollWheelZoom={true}
        className={`leaflet-map ${large ? "large-map" : ""}`}
      >
        <TileLayer
        ...
        {Object.keys(busRoutes).map((busId) => {
          const position =
            busRoutes[busId][busPositions[busId]];

          return (
            <Marker key={busId} position={position}>
              <Popup>
                <strong>{busId}</strong>
                <p>
                  Status:{" "}
...
function ActivityItem({
  ...
  return (
    <div className="activity">
      <div className="activity-icon">B</div>
...
function PriorityBadge({ type, children }) {
  return (
...
function AuthorityRow({ label, value }) {
  return (
...
function getAuthorityClass(status) {
  if (status === "Resolved") return "resolved";
  if (status === "Repair in Progress") return "progress";
